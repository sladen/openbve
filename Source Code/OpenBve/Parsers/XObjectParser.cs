    ...
    internal static class XObjectParser {
        ...
        internal static ObjectManager.StaticObject ReadObject(string FileName, System.Text.Encoding Encoding, ObjectManager.ObjectLoadMode LoadMode, bool ForceTextureRepeat) {
            ...
            if (Data[8] == 116 & Data[9] == 120 & Data[10] == 116 & Data[11] == 32) {
                ...
                try {
                    byte[] Uncompressed = new byte[65536];
                    int UncompressedLength = 0;
                    using (System.IO.MemoryStream Stream = new System.IO.MemoryStream(Data)) {
                        Stream.Position = 26;
                        using (System.IO.Compression.DeflateStream Zip = new System.IO.Compression.DeflateStream(Stream, System.IO.Compression.CompressionMode.Decompress)) {
                            while (true) {
                                if (UncompressedLength + 4096 >= Uncompressed.Length) {
                                    Array.Resize<byte>(ref Uncompressed, Uncompressed.Length << 1);
                                }
                                int n = Zip.Read(Uncompressed, UncompressedLength, 4096);
                                if (n == 0) break;
                    ...
                    Array.Resize<byte>(ref Uncompressed, UncompressedLength);
            ...
            for (int i = 0; i < Structure.Data.Length; i++) {
                ...
                switch (f.Name) {
                    case "Mesh": {
                            ...
                            Object.Meshes[m].Vertices = Vertices;
                            Object.Meshes[m].Faces = new World.MeshFace[nFaces];
                            Object.Meshes[m].Materials = new World.MeshMaterial[Materials.Length];
                            for (int j = 0; j < Materials.Length; j++) {
                                ...
                                if (Materials[j].TextureFilename != null) {
                                    ...
                                    Object.Meshes[m].Materials[j].DaytimeTextureIndex = tday;
                                    transparent = true;
                                } else {
                                    Object.Meshes[m].Materials[j].DaytimeTextureIndex = -1;
                                    transparent = false;
                                }
                                Object.Meshes[m].Materials[j].Flags = (byte)((transparent ? World.MeshMaterial.TransparentColorMask : 0) | (emissive ? World.MeshMaterial.EmissiveColorMask : 0));
                                Object.Meshes[m].Materials[j].Color = Materials[j].faceColor;
                                Object.Meshes[m].Materials[j].TransparentColor = new World.ColorRGB(0, 0, 0);
                                Object.Meshes[m].Materials[j].EmissiveColor = Materials[j].emissiveColor;
                                Object.Meshes[m].Materials[j].NighttimeTextureIndex = -1;
                                Object.Meshes[m].Materials[j].BlendMode = World.MeshMaterialBlendMode.Normal;
                                Object.Meshes[m].Materials[j].GlowAttenuationData = 0;
                            }
                            for (int j = 0; j < nFaces; j++) {
                                Object.Meshes[m].Faces[j].Material = (ushort)FaceMaterials[j];
                                Object.Meshes[m].Faces[j].Vertices = new World.MeshFaceVertex[Faces[j].Length];
                                bool impl = false;
                                bool expl = false;
                                for (int k = 0; k < Faces[j].Length; k++) {
                                    Object.Meshes[m].Faces[j].Vertices[k] = new World.MeshFaceVertex(Faces[j][k], FaceNormals[j][k]);
                                ...
                                Object.Meshes[m].Faces[j].Flags = impl & expl ? (byte)World.MeshFace.NormalValueMixed : expl ? (byte)World.MeshFace.NormalValueAllExplicit : (byte)World.MeshFace.NormalValueAllImplicit;
                            }
                            break;
                        }
                    case "Header":
