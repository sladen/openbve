    ...
    internal static class Interface {
        ...
        internal static string GetSettingsFolder() {
            if (Program.CurrentPlatform == Program.Platform.Windows) {
                return System.IO.Path.Combine(System.Windows.Forms.Application.StartupPath, "Settings");
            } else {
        ...
        internal static string GetDataFolder(params string[] Subfolders) {
            if (Program.CurrentPlatform == Program.Platform.Windows) {
                string Folder = System.IO.Path.Combine(System.Windows.Forms.Application.StartupPath, "Data");
                for (int i = 0; i < Subfolders.Length; i++) {
                    Folder = System.IO.Path.Combine(Folder, Subfolders[i]);
                }
                return Folder;
            } else {
                string Folder = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        ...
        internal static string GetControlsFolder() {
            if (Program.CurrentPlatform == Program.Platform.Windows) {
                return System.IO.Path.Combine(System.IO.Path.Combine(System.Windows.Forms.Application.StartupPath, "Data"), "Controls");
            } else {
        ...
        internal static void LoadOptions() {
            ...
            if (System.IO.File.Exists(File)) {
                            ...
                            switch (Section) {
                                ...
                                case "display":
                                    switch (Key) {
                                        ...
                                        case "userinterface": {
                                                Interface.CurrentOptions.UserInterfaceFolder = Value;
                                            } break;
                                    } break;
                                case "quality":
                                    switch (Key) {
                ...
                // file not found
                string Code = System.Globalization.CultureInfo.CurrentUICulture.Name;
        ...
        internal static void SaveOptions() {
            ...
            Builder.AppendLine("userinterface = " + CurrentOptions.UserInterfaceFolder);
            Builder.AppendLine();
        ...
        internal static Joystick[] CurrentJoysticks = new Joystick[] { };
        ...
        internal static void LoadControls(string FileOrNull) {
            ...
            CurrentControls = new Control[256];
            int Length = 0;
            System.Globalization.CultureInfo Culture = System.Globalization.CultureInfo.InvariantCulture;
            if (System.IO.File.Exists(File)) {
                string[] Lines = System.IO.File.ReadAllLines(File, new System.Text.UTF8Encoding());
                for (int i = 0; i < Lines.Length; i++) {
                    ...
                    if (Lines[i].Length != 0 && !Lines[i].StartsWith(";", StringComparison.OrdinalIgnoreCase)) {
                        ...
                        if (Terms.Length >= 2) {
                            if (Length >= CurrentControls.Length) {
                                Array.Resize<Control>(ref CurrentControls, CurrentControls.Length << 1);
                            }
                            int j;
                            ...
                            if (j == CommandInfos.Length) {
                                CurrentControls[Length].Command = Command.None;
                                CurrentControls[Length].InheritedType = CommandType.Digital;
                                CurrentControls[Length].Method = ControlMethod.Invalid;
                                CurrentControls[Length].Device = -1;
                                CurrentControls[Length].Component = JoystickComponent.Invalid;
                                CurrentControls[Length].Element = -1;
                                CurrentControls[Length].Direction = 0;
                                CurrentControls[Length].Modifier = KeyboardModifier.None;
                            } else {
                                CurrentControls[Length].Command = CommandInfos[j].Command;
                                CurrentControls[Length].InheritedType = CommandInfos[j].Type;
                                ...
                                if (Method == "keyboard" & Terms.Length == 4) {
                                    ...
                                    if (int.TryParse(Terms[2], System.Globalization.NumberStyles.Integer, Culture, out Element)) {
                                        if (int.TryParse(Terms[3], System.Globalization.NumberStyles.Integer, Culture, out Modifiers)) {
                                            CurrentControls[Length].Method = ControlMethod.Keyboard;
                                            CurrentControls[Length].Device = -1;
                                            CurrentControls[Length].Component = JoystickComponent.Invalid;
                                            CurrentControls[Length].Element = Element;
                                            CurrentControls[Length].Direction = 0;
                                            CurrentControls[Length].Modifier = (KeyboardModifier)Modifiers;
                                            Valid = true;
                                        }
                                    }
                                        ...
                                        if (Component == "axis" & Terms.Length == 6) {
                                            ...
                                            if (int.TryParse(Terms[4], System.Globalization.NumberStyles.Integer, Culture, out Element)) {
                                                if (int.TryParse(Terms[5], System.Globalization.NumberStyles.Integer, Culture, out Direction)) {
                                                    CurrentControls[Length].Method = ControlMethod.Joystick;
                                                    CurrentControls[Length].Device = Device;
                                                    CurrentControls[Length].Component = JoystickComponent.Axis;
                                                    CurrentControls[Length].Element = Element;
                                                    CurrentControls[Length].Direction = Direction;
                                                    CurrentControls[Length].Modifier = KeyboardModifier.None;
                                                    Valid = true;
                                                }
                                            }
                                            ...
                                            if (int.TryParse(Terms[4], System.Globalization.NumberStyles.Integer, Culture, out Element)) {
                                                if (int.TryParse(Terms[5], System.Globalization.NumberStyles.Integer, Culture, out Direction)) {
                                                    CurrentControls[Length].Method = ControlMethod.Joystick;
                                                    CurrentControls[Length].Device = Device;
                                                    CurrentControls[Length].Component = JoystickComponent.Ball;
                                                    CurrentControls[Length].Element = Element;
                                                    CurrentControls[Length].Direction = Direction;
                                                    CurrentControls[Length].Modifier = KeyboardModifier.None;
                                                    Valid = true;
                                            ...
                                            if (int.TryParse(Terms[4], System.Globalization.NumberStyles.Integer, Culture, out Element)) {
                                                if (int.TryParse(Terms[5], System.Globalization.NumberStyles.Integer, Culture, out Direction)) {
                                                    CurrentControls[Length].Method = ControlMethod.Joystick;
                                                    CurrentControls[Length].Device = Device;
                                                    CurrentControls[Length].Component = JoystickComponent.Hat;
                                                    CurrentControls[Length].Element = Element;
                                                    CurrentControls[Length].Direction = Direction;
                                                    CurrentControls[Length].Modifier = KeyboardModifier.None;
                                                    Valid = true;
                                            ...
                                            if (int.TryParse(Terms[4], System.Globalization.NumberStyles.Integer, Culture, out Element)) {
                                                CurrentControls[Length].Method = ControlMethod.Joystick;
                                                CurrentControls[Length].Device = Device;
                                                CurrentControls[Length].Component = JoystickComponent.Button;
                                                CurrentControls[Length].Element = Element;
                                                CurrentControls[Length].Direction = 0;
                                                CurrentControls[Length].Modifier = KeyboardModifier.None;
                                                Valid = true;
            ...
            Array.Resize<Control>(ref CurrentControls, Length);
        }
                                        ...
                                        case "value":
                                            if (Arguments.Length == 1) {
                                                ...
                                                if (!int.TryParse(Arguments[0], System.Globalization.NumberStyles.Integer, Culture, out n)) {
                                                    System.Windows.Forms.MessageBox.Show("VALUE is invalid in " + Command + " at line " + (i + 1).ToString(Culture) + " in " + File);
                                                } else {
