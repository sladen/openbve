	...
	internal static partial class Renderer {
		...
		internal static void RenderScene(double TimeElapsed) {
			...
			if (World.CurrentBackground.Texture != null) {
				Textures.LoadTexture(World.CurrentBackground.Texture);
			}
		...
		// render face
		private static Textures.Texture LastBoundTexture = null;
		...
		private static void RenderFace(ref World.MeshMaterial Material, World.Vertex[] Vertices, ref World.MeshFace Face, double CameraX, double CameraY, double CameraZ) {
			// texture
			if (Material.DaytimeTexture != null && Textures.LoadTexture(Material.DaytimeTexture)) {
				if (!TexturingEnabled) {
					Gl.glEnable(Gl.GL_TEXTURE_2D);
					TexturingEnabled = true;
				}
				if (Material.DaytimeTexture != LastBoundTexture) {
					Gl.glBindTexture(Gl.GL_TEXTURE_2D, Material.DaytimeTexture.OpenGlTextureName);
					LastBoundTexture = Material.DaytimeTexture;
				}
			} else {
				if (TexturingEnabled) {
			...
			if (Material.NighttimeTexture != null && Textures.LoadTexture(Material.NighttimeTexture)) {
				if (!TexturingEnabled) {
					Gl.glEnable(Gl.GL_TEXTURE_2D);
					TexturingEnabled = true;
				}
				...
				Gl.glBindTexture(Gl.GL_TEXTURE_2D, Material.NighttimeTexture.OpenGlTextureName);
				LastBoundTexture = null;
		...
		private static void RenderBackground(World.Background Data, double dx, double dy, double dz, float Alpha, float scale) {
			if (Data.Texture != null && Textures.LoadTexture(Data.Texture)) {
				if (LightingEnabled) {
					Gl.glDisable(Gl.GL_LIGHTING);
					LightingEnabled = false;
				}
				...
				Gl.glBindTexture(Gl.GL_TEXTURE_2D, Data.Texture.OpenGlTextureName);
		...
		private static void RenderFullscreenMotionBlur() {
			int w = Interface.CurrentOptions.NoTextureResize ? Screen.Width : Interface.RoundToPowerOfTwo(Screen.Width);
			int h = Interface.CurrentOptions.NoTextureResize ? Screen.Height : Interface.RoundToPowerOfTwo(Screen.Height);
			// render
		...
		private static void RenderOverlays(double TimeElapsed) {
			...
			if (CurrentOutputMode == OutputMode.Default) {
						...
						case "messages":
							{
								...
								if (Interface.CurrentHudElements[i].TopLeft.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].TopLeft.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].CenterLeft.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].CenterLeft.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].BottomLeft.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].BottomLeft.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].TopMiddle.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].TopMiddle.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].CenterMiddle.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].CenterMiddle.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].BottomMiddle.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].BottomMiddle.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].TopRight.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].TopRight.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].CenterRight.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].CenterRight.BackgroundTexture)) {
								...
								if (Interface.CurrentHudElements[i].BottomRight.BackgroundTexture != null) {
									if (Textures.LoadTexture(Interface.CurrentHudElements[i].BottomRight.BackgroundTexture)) {
								...
								for (int j = 0; j < n; j++) {
									...
									if (Left.BackgroundTexture != null) {
										if (Textures.LoadTexture(Left.BackgroundTexture)) {
									...
									if (Right.BackgroundTexture != null) {
										if (Textures.LoadTexture(Right.BackgroundTexture)) {
									...
									if (Middle.BackgroundTexture != null) {
										if (Textures.LoadTexture(Middle.BackgroundTexture)) {
									...
									{ /// text
										...
										DrawString(Interface.CurrentHudElements[i].Font, t, new System.Drawing.Point((int)p, (int)q), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(tr, tg, tb, ta * alpha), Interface.CurrentHudElements[i].TextShadow);
									}
									/// left overlay
									if (Left.OverlayTexture != null) {
										if (Textures.LoadTexture(Left.OverlayTexture)) {
									...
									if (Middle.OverlayTexture != null) {
										if (Textures.LoadTexture(Middle.OverlayTexture)) {
								...
								for (int j = 0; j < n; j++) {
									if (CurrentLampCollection.Lamps[j].Type != LampType.None) {
										...
										{ /// text
											...
											p += Interface.CurrentHudElements[i].TextPosition.X;
											q += Interface.CurrentHudElements[i].TextPosition.Y;
											DrawString(Interface.CurrentHudElements[i].Font, t, new System.Drawing.Point((int)p, (int)q), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(tr, tg, tb, ta), Interface.CurrentHudElements[i].TextShadow);
										}
										/// left overlay
										if (Left.OverlayTexture != null) {
											if (Textures.LoadTexture(Left.OverlayTexture)) {
						...
						default:
							{
								...
								if (alpha != 0.0f) {
									...
									{ // text
										...
										DrawString(Interface.CurrentHudElements[i].Font, t, new System.Drawing.Point((int)p, (int)q), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(r, g, b, a * alpha), Interface.CurrentHudElements[i].TextShadow);
									}
									// overlay
									if (Interface.CurrentHudElements[i].CenterMiddle.OverlayTexture != null) {
										if (Textures.LoadTexture(Interface.CurrentHudElements[i].CenterMiddle.OverlayTexture)) {
											float r, g, b, a;
				...
				if (Interface.CurrentOptions.GameMode != Interface.GameMode.Expert) {
					...
					for (int i = 0; i < Game.MarkerTextures.Length; i++) {
						if (Textures.LoadTexture(Game.MarkerTextures[i])) {
				...
				if (Timetable.CurrentTimetable == Timetable.TimetableState.Default) {
					// default
					if (Textures.LoadTexture(Timetable.DefaultTimetableTexture)) {
					...
					// custom
					if (Textures.LoadTexture(Timetable.CurrentCustomTimetableDaytimeTexture)) {
				...
				// actual handles
				{
					...
					DrawString(Fonts.SmallFont, t, new System.Drawing.Point(2, Screen.Height - 46), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
				}
				// safety handles
				{
					...
					DrawString(Fonts.SmallFont, t, new System.Drawing.Point(2, Screen.Height - 32), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
				}
				// driver handles
				{
					...
					DrawString(Fonts.SmallFont, t, new System.Drawing.Point(2, Screen.Height - 18), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
				}
				// debug information
				...
				for (int i = 0; i < Lines.Length; i++) {
					if (Lines[i].Length != 0) {
						if (Lines[i][0] == '=') {
							...
							DrawString(Fonts.SmallFont, Lines[i], new System.Drawing.Point((int)x, (int)y), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White);
						} else {
							DrawString(Fonts.SmallFont, Lines[i], new System.Drawing.Point((int)x, (int)y), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
						}
						y += 14.0;
			...
			if (Interface.CurrentOptions.GameMode != Interface.GameMode.Expert & OptionBrakeSystems) {
				...
				for (int i = 0; i < TrainManager.PlayerTrain.Cars.Length; i++) {
					...
					if (TrainManager.PlayerTrain.Cars[i].Specs.BrakeType == TrainManager.CarBrakeType.AutomaticAirBrake | TrainManager.PlayerTrain.Cars[i].Specs.BrakeType == TrainManager.CarBrakeType.ElectromagneticStraightAirBrake) {
						if (!heading[0]) {
							DrawString(Fonts.SmallFont, "Brake pipe", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[0] = true;
						}
					...
					if (TrainManager.PlayerTrain.Cars[i].Specs.BrakeType == TrainManager.CarBrakeType.AutomaticAirBrake | TrainManager.PlayerTrain.Cars[i].Specs.BrakeType == TrainManager.CarBrakeType.ElectromagneticStraightAirBrake) {
						if (!heading[1]) {
							...
							DrawString(Fonts.SmallFont, "Auxillary reservoir", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[1] = true;
						}
						Gl.glColor3f(0.0f, 0.0f, 0.0f);
						RenderOverlaySolid(x, y, x + w, y + h);
					...
					// brake cylinder
					{
						if (!heading[2]) {
							...
							DrawString(Fonts.SmallFont, "Brake cylinder", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[2] = true;
						}
						Gl.glColor3f(0.0f, 0.0f, 0.0f);
						RenderOverlaySolid(x, y, x + w, y + h);
					...
					if (TrainManager.PlayerTrain.Cars[i].Specs.AirBrake.Type == TrainManager.AirBrakeType.Main) {
						if (!heading[3]) {
							...
							DrawString(Fonts.SmallFont, "Main reservoir", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[3] = true;
						}
						Gl.glColor3f(0.0f, 0.0f, 0.0f);
						RenderOverlaySolid(x, y, x + w, y + h);
					...
					if (TrainManager.PlayerTrain.Cars[i].Specs.AirBrake.Type == TrainManager.AirBrakeType.Main) {
						if (!heading[4]) {
							...
							DrawString(Fonts.SmallFont, "Equalizing reservoir", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[4] = true;
						}
						Gl.glColor3f(0.0f, 0.0f, 0.0f);
						RenderOverlaySolid(x, y, x + w, y + h);
					...
					if (TrainManager.PlayerTrain.Cars[i].Specs.BrakeType == TrainManager.CarBrakeType.ElectromagneticStraightAirBrake & TrainManager.PlayerTrain.Cars[i].Specs.AirBrake.Type == TrainManager.AirBrakeType.Main) {
						if (!heading[5]) {
							...
							DrawString(Fonts.SmallFont, "Straight air pipe", new System.Drawing.Point((int)x, (int)(oy - 16)), TextAlignment.TopLeft, OpenBveApi.Objects.Color128.White, true);
							heading[5] = true;
						}
						Gl.glColor3f(0.0f, 0.0f, 0.0f);
						RenderOverlaySolid(x, y, x + w, y + h);
			...
			if (Game.CurrentInterface == Game.InterfaceType.Pause) {
				...
				Gl.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
				DrawString(Fonts.VeryLargeFont, "PAUSE", new System.Drawing.Point(Screen.Width / 2, Screen.Height / 2), TextAlignment.CenterMiddle, OpenBveApi.Objects.Color128.White, true);
				...
				for (int i = 0; i < Game.CurrentMenuSelection.Length; i++) {
					...
					for (int j = 0; j < m.Length; j++) {
						...
						if (j == Game.CurrentMenuSelection[i]) {
							DrawString(Fonts.NormalFont, "➢", new System.Drawing.Point((int)x, (int)y), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(r, g, b, a), true);
						}
						if (m[j] is Game.MenuCaption) {
							DrawString(Fonts.NormalFont, m[j].Text, new System.Drawing.Point((int)x + 24, (int)y), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(0.5f, 0.75f, 1.0f, a), true);
						} else if (m[j] is Game.MenuCommand) {
							DrawString(Fonts.NormalFont, m[j].Text, new System.Drawing.Point((int)x + 24, (int)y), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(r, g, b, a), true);
						} else {
							DrawString(Fonts.NormalFont, " ➟", new System.Drawing.Point((int)x + 24, (int)y), TextAlignment.TopLeft, new OpenBveApi.Objects.Color128(r, g, b, a), true);
						}
						y += h;
					}
					x += w;
					...
					if (Type == ObjectType.Overlay & World.CameraRestriction != World.CameraRestrictionMode.NotAvailable) {
						...
						if (ObjectManager.Objects[ObjectIndex].Mesh.Materials[k].Color.A != 255) {
							...
							if (ObjectManager.Objects[ObjectIndex].Mesh.Materials[k].DaytimeTexture != null) {
								if (Textures.LoadTexture(ObjectManager.Objects[ObjectIndex].Mesh.Materials[k].DaytimeTexture)) {
							...
							if (ObjectManager.Objects[ObjectIndex].Mesh.Materials[k].NighttimeTexture != null) {
								if (Textures.LoadTexture(ObjectManager.Objects[ObjectIndex].Mesh.Materials[k].NighttimeTexture)) {
