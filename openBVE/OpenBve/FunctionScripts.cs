...
namespace OpenBve {
    internal static class FunctionScripts {
        ...
        internal enum Instructions : int {
            SystemHalt, SystemConstant, SystemValue, SystemDelta,
            StackCopy, StackSwap,
            ...
            for (int i = 0; i < Arguments.Length; i++) {
                double d; if (double.TryParse(Arguments[i], System.Globalization.NumberStyles.Float, Culture, out d)) {
                    ...
                    switch (Arguments[i].ToLowerInvariant()) {
                        ...
                        // stack
                        case "~":
                            if (s < 1) throw new System.InvalidOperationException(Arguments[i] + " requires at least one argument on the stack in function script " + Expression);
                            if (n >= Result.Instructions.Length) Array.Resize<Instructions>(ref Result.Instructions, Result.Instructions.Length << 1);
                            Result.Instructions[n] = Instructions.StackCopy;
                            n++; s++; if (s >= m) m = s; break;
                        case "<>":
                            if (s < 2) throw new System.InvalidOperationException(Arguments[i] + " requires at least two arguments on the stack in function script " + Expression);
                            if (n >= Result.Instructions.Length) Array.Resize<Instructions>(ref Result.Instructions, Result.Instructions.Length << 1);
                        ...
                        case "--":
                            ...
                            n++; break;
                        case "fma":
                            if (s < 3) throw new System.InvalidOperationException(Arguments[i] + " requires at least three arguments on the stack in function script " + Expression);
                            if (n >= Result.Instructions.Length) Array.Resize<Instructions>(ref Result.Instructions, Result.Instructions.Length << 1);
