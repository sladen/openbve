				...
				for (int j = 0; j < CustomObjects[i].States.Length; j++) {
					for (int k = 0; k < CustomObjects[i].States[j].Object.Mesh.Materials.Length; k++) {
						if (DaytimeTextureIndex >= 0) {
							CustomObjects[i].States[j].Object.Mesh.Materials[k].DaytimeTextureIndex = DaytimeTextureIndex;
						}
						if (NighttimeTextureIndex >= 0) {
							CustomObjects[i].States[j].Object.Mesh.Materials[k].NighttimeTextureIndex = NighttimeTextureIndex;
						}
					}
				}
			}
