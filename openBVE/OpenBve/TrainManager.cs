...
namespace OpenBve {
    internal static class TrainManager {
        ...
        // trains
        internal static Train[] Trains;
        internal static int PlayerTrain;
        ...
        internal static void ParsePanelConfig(string TrainPath, System.Text.Encoding Encoding, TrainManager.Train Train, bool OverlayMode) {
            ...
            if (System.IO.File.Exists(File)) {
                BvePanel2CfgParser.ParsePanel2Config(TrainPath, Encoding, Train);
            } else {
                BvePanelCfgParser.ParsePanelConfig(TrainPath, Encoding, Train);
            }
        }
        ...
        internal static void UpdateTopplingCantAndSpring(Train Train, int CarIndex, double TimeElapsed) {
            ...
            if (Interface.CurrentOptions.Toppling | Train.Cars[CarIndex].Derailed) {
                ...
                double s = Train.Cars[CarIndex].Specs.CurrentSpeed;
            ...
            // spring sound
            {
                ...
                if (diff < -0.0275) {
                    ...
                    if (snd >= 0) {
                        ...
                        SoundManager.PlaySound(snd, Train.TrainIndex, CarIndex, pos, SoundManager.Importance.DontCare, false);
                    }
            ...
            // flange sound
            {
                World.Vector3D g = World.Vector3D.Subtract(Train.Cars[CarIndex].FrontAxle.Follower.WorldPosition, Train.Cars[CarIndex].RearAxle.Follower.WorldPosition);
                double b0 = g.X * Train.Cars[CarIndex].RearAxle.Follower.WorldSide.X + g.Y * Train.Cars[CarIndex].RearAxle.Follower.WorldSide.Y + g.Z * Train.Cars[CarIndex].RearAxle.Follower.WorldSide.Z;
                double b1 = g.X * Train.Cars[CarIndex].FrontAxle.Follower.WorldSide.X + g.Y * Train.Cars[CarIndex].FrontAxle.Follower.WorldSide.Y + g.Z * Train.Cars[CarIndex].FrontAxle.Follower.WorldSide.Z;
                const double refspd = 12.5; /// 45 km/h => m/s
                const double invrefspd = 0.08; /// 45 km/h => m/s => 1/x
                const double reffac = 0.5;
                double spd = Math.Abs(Train.Cars[CarIndex].Specs.CurrentSpeed);
                double pitch = (reffac * (spd - refspd) + refspd) * invrefspd;
                double b2 = Math.Abs(b0) + Math.Abs(b1);
                double basegain = 0.075 * b2 * spd;
                ...
                for (int i = 0; i < Train.Cars[CarIndex].Sounds.Flange.Length; i++) {
                    ...
                    if (Train.Cars[CarIndex].Sounds.Flange[i].SoundSourceIndex >= 0) {
                        ...
                        if (snd >= 0) {
                            ...
                            SoundManager.PlaySound(ref Train.Cars[CarIndex].Sounds.Flange[i].SoundSourceIndex, snd, Train.TrainIndex, CarIndex, pos, SoundManager.Importance.DontCare, true, pitch, gain);
                        }
                    }
                }
            }
        ...
        private static void UpdateCar(Train Train, int CarIndex, double TimeElapsed) {
            ...
            if (s >= 0) {
                ...
                for (int i = 0; i < Train.Cars[c].Sections[s].Elements.Length; i++) {
                    ...
                    for (int j = 0; j < ObjectManager.Objects[o].Meshes.Length; j++) {
                        for (int k = 0; k < ObjectManager.Objects[o].Meshes[j].Materials.Length; k++) {
                            ObjectManager.Objects[o].Meshes[j].Materials[k].DaytimeNighttimeBlend = dnb;
                        }
                    }
                }
            }
                            ...
                            if (!Trains[j].Disposed & !Trains[j].IsBogusTrain) {
                                ...
                                if (c >= b & c <= a) {
                                    ...
                                    if (Trains[i].Cars[k].Specs.CurrentSpeed < Trains[j].Cars[0].Specs.CurrentSpeed) {
                                        ...
                                        double e = 0.5 * (c - b);
                                        Trains[i].Cars[k].FrontAxle.Follower.TrackPosition += e;
                                        Trains[i].Cars[k].RearAxle.Follower.TrackPosition += e;
                                        Trains[j].Cars[0].FrontAxle.Follower.TrackPosition -= e;
                                        Trains[j].Cars[0].RearAxle.Follower.TrackPosition -= e;
                                        ...
                                        double vi = v * fi;
                                        double vj = v * fj;
                                        //Trains[i].Cars[k].Specs.CurrentRollShakeDirection += vi;
                                        //Trains[j].Cars[0].Specs.CurrentRollShakeDirection += vj;
                                    ...
                                    if (Trains[i].Cars[0].Specs.CurrentSpeed > Trains[j].Cars[k].Specs.CurrentSpeed) {
                                        ...
                                        double e = 0.5 * (a - d);
                                        Trains[i].Cars[0].FrontAxle.Follower.TrackPosition -= e;
                                        Trains[i].Cars[0].RearAxle.Follower.TrackPosition -= e;
                                        Trains[j].Cars[k].FrontAxle.Follower.TrackPosition += e;
                                        Trains[j].Cars[k].RearAxle.Follower.TrackPosition += e;
                                        ...
                                        double vi = v * fi;
                                        double vj = v * fj;
                                        //Trains[i].Cars[0].Specs.CurrentRollShakeDirection += vi;
                                        //Trains[j].Cars[k].Specs.CurrentRollShakeDirection += vj;
                                        if (Interface.CurrentOptions.Derailments) {
        ...
        internal static void DisposeTrain(Train Train) {
            if (!Train.Disposed) {
                ...
                SoundManager.StopAllSounds(Train.TrainIndex, true);
                for (int i = 0; i < Game.Sections.Length; i++) {
                    Game.Sections[i].Leave(Train.TrainIndex);
                }
        ...
        private static void UpdateTrainStation(Train Train, double TimeElapsed) {
            if (Train.Station >= 0) {
                ...
                if (n >= 0) {
                    ...
                    Train.StationStopDifference = p1 - p0;
                } else {
                        ...
                        if (Game.SecondsSinceMidnight > Train.StationDepartureTime) {
                            ...
                            if (Train.TrainIndex == PlayerTrain & !Game.Stations[i].IsTerminalStation) {
                                if (!Game.Stations[i].OpenLeftDoors & !Game.Stations[i].OpenRightDoors | Train.Specs.DoorCloseMode != DoorMode.Manual) {
                                    ...
                                    Game.AddMessage(Interface.GetInterfaceString("message_station_depart_closedoors"), Game.MessageDependency.None, Game.MessageType.Game, Game.MessageColor.Green, Game.SecondsSinceMidnight + 5.0);
                                }
                            }
                        }
                        // passengers boarding
            ...
            if (sl) {
                for (int i = 0; i < Train.Cars.Length; i++) {
                    ...
                    if (snd >= 0) {
                        ...
                        SoundManager.PlaySound(snd, Train.TrainIndex, i, pos, SoundManager.Importance.DontCare, false);
                    }
                }
            }
            if (sr) {
        ...
        internal static bool ExistNonOpenDoors(Train Train, bool Left, bool Right) {
            for (int i = 0; i < Train.Cars.Length; i++) {
                for (int j = 0; j < Train.Cars[i].Specs.Doors.Length; j++) {
                    if (Left & Train.Cars[i].Specs.Doors[j].Direction == -1) {
                        if (Train.Cars[i].Specs.Doors[j].State != 1.0) return true;
                    }
                    if (Right & Train.Cars[i].Specs.Doors[j].Direction == 1) {
                        if (Train.Cars[i].Specs.Doors[j].State != 1.0) return true;
                    }
                }
            }
        ...
        internal static bool ExistNonClosedDoors(Train Train, bool Left, bool Right) {
            for (int i = 0; i < Train.Cars.Length; i++) {
                for (int j = 0; j < Train.Cars[i].Specs.Doors.Length; j++) {
                    if (Left & Train.Cars[i].Specs.Doors[j].Direction == -1) {
                        if (Train.Cars[i].Specs.Doors[j].State != 0.0) return true;
                    }
                    if (Right & Train.Cars[i].Specs.Doors[j].Direction == 1) {
                        if (Train.Cars[i].Specs.Doors[j].State != 0.0) return true;
                    }
        ...
        private static void UpdateSecuritySystem(Train Train, double TimeElapsed) {
            // plugin
            if (Train.Specs.Security.Mode == SecuritySystem.Bve4Plugin) {
                while (Train.Specs.Security.IsTransponderPending()) {
                    TrainPendingTransponder Data = Train.Specs.Security.GetPendingTransponder();
            ...
            if (Train.Specs.Security.Mode != SecuritySystem.None) {
                ...
                if (Train.Specs.Security.Eb.Available) {
                    if (Train.Specs.CurrentEmergencyBrake.Driver | Game.MinimalisticSimulation & Train.TrainIndex != PlayerTrain) {
                        ...
                        if (Train.Specs.Security.Eb.BellState == SecurityState.Normal) {
                            if (Game.SecondsSinceMidnight - Train.Specs.Security.Eb.Time >= 60.0 & Math.Abs(Train.Cars[Train.DriverCar].Specs.CurrentPerceivedSpeed) >= 4.16666666666667) {
                                ...
                                if (snd >= 0) {
                                    ...
                                    SoundManager.PlaySound(ref Train.Cars[Train.DriverCar].Sounds.Eb.SoundSourceIndex, snd, Train.TrainIndex, Train.DriverCar, pos, SoundManager.Importance.DontCare, true);
                                }
                ...
                if (Train.Specs.Security.State == SecurityState.Initialization | Train.Specs.Security.State == SecurityState.Ringing | Train.Specs.Security.State == SecurityState.Emergency) {
                    if (Train.Specs.Security.State == SecurityState.Initialization & Game.SecondsSinceMidnight - Train.Specs.Security.Ats.Time >= 0.5) {
                        ...
                        if (snd >= 0 & Train.Cars[Train.DriverCar].Sounds.AtsCnt.SoundSourceIndex < 0) {
                            ...
                            SoundManager.PlaySound(ref Train.Cars[Train.DriverCar].Sounds.AtsCnt.SoundSourceIndex, snd, Train.TrainIndex, Train.DriverCar, pos, SoundManager.Importance.DontCare, true);
                        }
                    ...
                    }
                    {
                        ...
                        if (snd >= 0) {
                            if (!SoundManager.IsPlaying(Train.Cars[Train.DriverCar].Sounds.Ats.SoundSourceIndex)) {
                                ...
                                SoundManager.PlaySound(ref Train.Cars[Train.DriverCar].Sounds.Ats.SoundSourceIndex, snd, Train.TrainIndex, Train.DriverCar, pos, SoundManager.Importance.DontCare, true);
                            }
                        }
                    }
                    ...
                    if (Train.Cars[CarIndex].Specs.AirBrake.MainReservoirCurrentPressure > Train.Cars[CarIndex].Specs.AirBrake.AirCompressorMaximumPressure) {
                        ...
                        if (!Train.Cars[CarIndex].Sounds.CpLoopStarted && Game.SecondsSinceMidnight > Train.Cars[CarIndex].Sounds.CpStartTimeStarted + 5.0) {
                            ...
                            if (snd >= 0) {
                                ...
                                SoundManager.PlaySound(ref Train.Cars[CarIndex].Sounds.CpLoop.SoundSourceIndex, snd, Train.TrainIndex, CarIndex, pos, SoundManager.Importance.DontCare, true);
                            }
                        }
                    }
                } else {
            ...
            { // rub sound
                ...
                if (snd >= 0) {
                    ...
                    if (Train.Cars[CarIndex].Sounds.Rub.SoundSourceIndex >= 0) {
                        ...
                        SoundManager.PlaySound(ref Train.Cars[CarIndex].Sounds.Rub.SoundSourceIndex, snd, Train.TrainIndex, CarIndex, Train.Cars[CarIndex].Sounds.Rub.Position, SoundManager.Importance.DontCare, true, pitch, gain);
                    }
                }
            }
        }
        ...
        internal static void ApplyNotch(Train Train, int PowerValue, bool PowerRelative, int BrakeValue, bool BrakeRelative) {
            ...
            //if (p == Train.Specs.CurrentPowerNotch.Driver & b == Train.Specs.CurrentBrakeNotch.Driver) return;
            // power sound
        ...
        private static void UpdateTrainPassengers(Train Train, double TimeElapsed) {
            ...
            if (Math.Abs(Train.Passengers.CurrentAcceleration) > 2.0 * spdmax) {
                ...
                //Game.AddMessage("PASSENGER DISCOMFORT", Game.MessageDependency.None, Game.MessageType.Interface, Game.MessageColor.Magenta, Game.SecondsSinceMidnight + 1.5);
            } else {
        ...
        private static void UpdateSpeeds(Train Train, double TimeElapsed) {
            if (Game.MinimalisticSimulation & Train.TrainIndex == PlayerTrain) {
        ...
        private static void UpdateTrainPhysicsAndControls(Train Train, double TimeElapsed) {
            ...
            if (!Game.MinimalisticSimulation & Train.TrainIndex == PlayerTrain) {
                if (World.CameraMode == World.CameraViewMode.Interior | World.CameraMode == World.CameraViewMode.Exterior) {
                    if (World.CameraMode != World.CameraViewMode.Interior | !TrainManager.Trains[TrainManager.PlayerTrain].Cars[0].Sections[0].Overlay) {
                        UpdateCamera(Train);
                    }
                }
            }
            ...
            { /// air brake handle
                if (Train.Specs.AirBrake.Handle.DelayedValue != AirBrakeHandleState.Invalid) {
                    ...
                    if (Train.Specs.AirBrake.Handle.Security == AirBrakeHandleState.Release & Train.Specs.AirBrake.Handle.Actual != AirBrakeHandleState.Release) {
                        ...
                        Train.Specs.AirBrake.Handle.DelayedTime = Game.SecondsSinceMidnight; // +Train.Specs.DelayServiceBrake;
            ...
            { /// emergency brake
                if (Train.Specs.CurrentEmergencyBrake.Security & !Train.Specs.CurrentEmergencyBrake.Actual) {
                    double t = Game.SecondsSinceMidnight; // +Train.Specs.DelayEmergencyBrake;
                ...
                for (int j = 0; j < Train.Cars[i].Sounds.Run.Length; j++) {
                    ...
                    if (Train.Cars[i].Sounds.Run[j].SoundSourceIndex >= 0) {
                        ...
                        if (snd >= 0) {
                            ...
                            SoundManager.PlaySound(ref Train.Cars[i].Sounds.Run[j].SoundSourceIndex, snd, Train.TrainIndex, i, pos, SoundManager.Importance.DontCare, true, pitch, gain);
                        }
                    }
                }
            }
                            ...
                            if (j < Train.Cars[i].Sounds.Motor.Tables.Length) {
                                ...
                                if (idx2 >= 0) {
                                    ...
                                    if (obuf != nbuf) {
                                        ...
                                        Train.Cars[i].Sounds.Motor.Tables[j].SoundBufferIndex = -1;
                                        if (nbuf >= 0) {
                                            SoundManager.PlaySound(ref Train.Cars[i].Sounds.Motor.Tables[j].SoundSourceIndex, nbuf, Train.TrainIndex, i, pos, SoundManager.Importance.DontCare, true, pitch, gain);
            ...
            if (Train.CurrentSectionLimit == 0.0) {
                if (Train.Specs.CurrentEmergencyBrake.Driver & Train.Specs.CurrentAverageSpeed > -0.05 & Train.Specs.CurrentAverageSpeed < 0.05) {
                    ...
                    if (Train.TrainIndex == PlayerTrain) {
                        ...
                        Game.AddMessage(s, Game.MessageDependency.None, Game.MessageType.Game, Game.MessageColor.Red, Game.SecondsSinceMidnight + 5.0);
                    }
                }
            }
            // infrequent updates
