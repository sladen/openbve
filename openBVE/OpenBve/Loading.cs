...
namespace OpenBve {
    internal static class Loading {
        ...
        private static void LoadEverythingThreaded() {
            ...
            World.CameraTrackFollower.TrainIndex = -1;
            World.CameraTrackFollower.CarIndex = -1;
            World.CameraMode = World.CameraViewMode.Interior;
            // load route
            BveCsvRwRouteParser.ParseRoute(CurrentRouteFile, CurrentRouteEncoding, CurrentTrainFolder, ObjectFolder, SoundFolder, CompatibilityFolder, false);
            ...
            TrainManager.PlayerTrain = Game.PretrainsUsed;
            // load trains
            double TrainProgressMaximum = 0.7 + 0.3 * (double)TrainManager.Trains.Length;
            for (int k = 0; k < TrainManager.Trains.Length; k++) {
                bool bogus = k != TrainManager.PlayerTrain & Game.BogusPretrainInstructions.Length > 0;
                TrainManager.Trains[k] = new TrainManager.Train();
                TrainManager.Trains[k].TrainIndex = k;
                TrainManager.Trains[k].IsBogusTrain = bogus;
                if (bogus) {
                    // bogus train
                    string Folder = Interface.GetCombinedFolderName(Interface.GetCombinedFolderName(Application.StartupPath, "Compatibility"), "pretrain");
                    BveTrainDatParser.ParseTrainData(Folder, System.Text.Encoding.UTF8, TrainManager.Trains[k]);
                    System.Threading.Thread.Sleep(1); if (Cancel) return;
                    BveSoundCfgParser.LoadNoSound(TrainManager.Trains[k]);
                    System.Threading.Thread.Sleep(1); if (Cancel) return;
                    ...
                    BveTrainDatParser.ParseTrainData(CurrentTrainFolder, CurrentTrainEncoding, TrainManager.Trains[k]);
                    TrainProgressCurrentSum += TrainProgressCurrentWeight;
                    System.Threading.Thread.Sleep(1); if (Cancel) return;
                    TrainProgressCurrentWeight = 0.2 / TrainProgressMaximum;
                    BveSoundCfgParser.ParseSoundConfig(CurrentTrainFolder, CurrentTrainEncoding, TrainManager.Trains[k]);
                    TrainProgressCurrentSum += TrainProgressCurrentWeight;
                    System.Threading.Thread.Sleep(1); if (Cancel) return;
                ...
                for (int i = 0; i < TrainManager.Trains[k].Cars.Length; i++) {
                    TrainManager.Trains[k].Cars[i].FrontAxle.Follower.TrainIndex = k;
                    TrainManager.Trains[k].Cars[i].RearAxle.Follower.TrainIndex = k;
                }
                // add panel section
                if (k == TrainManager.PlayerTrain) {
                ...
                if (!bogus) {
                    ...
                    BveExtensionsCfgParser.ParseExtensionsConfig(CurrentTrainFolder, CurrentTrainEncoding, out CarObjects, TrainManager.Trains[k]);
                    System.Threading.Thread.Sleep(1); if (Cancel) return;
                    for (int i = 0; i < TrainManager.Trains[k].Cars.Length; i++) {
                        if (CarObjects[i] == null & Interface.CurrentOptions.ShowDefaultExteriorObjects) {
                            ...
                            o.Meshes = new World.Mesh[1];
                            o.Meshes[0].Vertices = new World.Vertex[8];
                            ...
                            o.Meshes[0].Vertices[0].Coordinates = new World.Vector3D(sx, sy, -sz);
                            o.Meshes[0].Vertices[1].Coordinates = new World.Vector3D(sx, 0.0, -sz);
                            o.Meshes[0].Vertices[2].Coordinates = new World.Vector3D(-sx, 0.0, -sz);
                            o.Meshes[0].Vertices[3].Coordinates = new World.Vector3D(-sx, sy, -sz);
                            o.Meshes[0].Vertices[4].Coordinates = new World.Vector3D(sx, sy, sz);
                            o.Meshes[0].Vertices[5].Coordinates = new World.Vector3D(sx, 0.0, sz);
                            o.Meshes[0].Vertices[6].Coordinates = new World.Vector3D(-sx, 0.0, sz);
                            o.Meshes[0].Vertices[7].Coordinates = new World.Vector3D(-sx, sy, sz);
                            o.Meshes[0].Materials = new World.MeshMaterial[1];
                            o.Meshes[0].Materials[0] = new World.MeshMaterial();
                            o.Meshes[0].Materials[0].Color = new World.ColorRGBA(192, 192, 192, 255);
                            o.Meshes[0].Materials[0].DaytimeTextureIndex = -1;
                            o.Meshes[0].Materials[0].NighttimeTextureIndex = -1;
                            o.Meshes[0].Faces = new World.MeshFace[6];
                            o.Meshes[0].Faces[0] = new World.MeshFace(new int[] { 0, 1, 2, 3 });
                            o.Meshes[0].Faces[1] = new World.MeshFace(new int[] { 0, 4, 5, 1 });
                            o.Meshes[0].Faces[2] = new World.MeshFace(new int[] { 0, 3, 7, 4 });
                            o.Meshes[0].Faces[3] = new World.MeshFace(new int[] { 6, 5, 4, 7 });
                            o.Meshes[0].Faces[4] = new World.MeshFace(new int[] { 6, 7, 3, 2 });
                            o.Meshes[0].Faces[5] = new World.MeshFace(new int[] { 6, 2, 1, 5 });
                            CarObjects[i] = o;
                        }
                ...
                if (k == TrainManager.PlayerTrain) {
                    ...
                    TrainManager.Trains[k].AI = new Game.BogusPretrainAI();
                } else {
            ...
            for (int i = 0; i < Game.Stations.Length; i++) {
                if (Game.Stations[i].Stops.Length != 0) {
                    FirstStationIndex = i;
                    int s = Game.GetStopIndex(i, TrainManager.Trains[TrainManager.PlayerTrain].Cars.Length);
                    if (s >= 0) {
            ...
            for (int i = 0; i < TrainManager.Trains.Length; i++) {
                ...
                if (TrainManager.Trains[i].AI != null) {
                    TrainManager.Trains[i].AI.Initialize(TrainManager.Trains[i]);
                }
                for (int j = 0; j < TrainManager.Trains[i].Cars.Length; j++) {
                    if (TrainManager.Trains[i].Cars[j].Sections.Length > 0) {
                        TrainManager.ChangeCarSection(TrainManager.Trains[i], j, j == 0 | i != TrainManager.PlayerTrain ? 0 : -1);
                    }
                }
                ...
                if (TrainManager.Trains[i].IsBogusTrain & Game.BogusPretrainInstructions.Length != 0) {
                    ...
                    p = i == TrainManager.PlayerTrain ? TrackPosition : FirstStationPosition;
                }
                double d = p - TrainManager.Trains[i].Cars[0].FrontAxle.Follower.TrackPosition;
                for (int j = 0; j < TrainManager.Trains[i].Cars.Length; j++) {
                    TrainManager.MoveCar(TrainManager.Trains[i], j, 0.1, 0.1);
                    TrainManager.MoveCar(TrainManager.Trains[i], j, -0.1, 0.1);
            ...
            if (TrainManager.PlayerTrain >= 0) {
                PluginManager.LoadAtsConfig(CurrentTrainFolder, CurrentTrainEncoding, TrainManager.Trains[TrainManager.PlayerTrain]);
            }
            // time table
            System.Threading.Thread.Sleep(1); if (Cancel) return;
            ...
            // initialize camera
            System.Threading.Thread.Sleep(1); if (Cancel) return;
            TrainManager.UpdateCamera(TrainManager.Trains[TrainManager.PlayerTrain]);
