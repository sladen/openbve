...
namespace OpenBve {
	...
	internal static partial class ManagedContent {
		...
		internal class Version {
			...
			/// <param name="languageCode">The preferred language code.</param>
		...
		internal partial class Database {
			...
			/// <param name="exactVersion">Whether to compare against the exact version or at least the specified version.</param>
			/// <returns>The package version, or a null reference if not found.</returns>
			internal Version Dereference(string name, string version, bool exactVersion) {
				for (int i = 0; i < this.Packages.Length; i++) {
					if (string.Equals(this.Packages[i].Name, name, StringComparison.OrdinalIgnoreCase)) {
						for (int j = 0; j < this.Packages[i].Versions.Length; j++) {
							int delta = CompareVersions(this.Packages[i].Versions[j].Number, version);
							if (delta == 0 | delta == 1 & !exactVersion) {
								return this.Packages[i].Versions[j];
							}
						}
						return null;
					}
		...
		/// <param name="key">The key without the language code.</param>
		/// <param name="preferredLanguage">The preferred language code.</param>
		/// <param name="defaultValue">The default value in case the key is not found.</param>
		/// <returns>The value.</returns>
