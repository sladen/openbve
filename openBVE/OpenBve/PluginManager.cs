                ...
                if (PluginSound[i] != PluginSoundCache[i]) {
                    if (PluginSound[i] == ATS_SOUND_STOP) {
                        ...
                        if (i < Train.Cars[Train.DriverCar].Sounds.Plugin.Length) {
                            ...
                            if (snd >= 0) {
                                ...
                                if (SoundManager.IsPlaying(Train.Cars[Train.DriverCar].Sounds.Plugin[i].SoundSourceIndex)) {
                                    ...
                                    SoundManager.PlaySound(ref Train.Cars[Train.DriverCar].Sounds.Plugin[i].SoundSourceIndex, snd, Train.TrainIndex, Train.DriverCar, Train.Cars[Train.DriverCar].Sounds.Plugin[i].Position, SoundManager.Importance.AlwaysPlay, true, 1.0, gain);
                                }
                            }
                        }
                        ...
                        if (i < Train.Cars[Train.DriverCar].Sounds.Plugin.Length) {
                            int snd = Train.Cars[Train.DriverCar].Sounds.Plugin[i].SoundBufferIndex;
                            if (snd >= 0) {
                                SoundManager.PlaySound(ref Train.Cars[Train.DriverCar].Sounds.Plugin[i].SoundSourceIndex, snd, Train.TrainIndex, Train.DriverCar, Train.Cars[Train.DriverCar].Sounds.Plugin[i].Position, SoundManager.Importance.AlwaysPlay, false);
                            }
                        }
        ...
        internal static void UpdateBeacon(TrainManager.Train Train, TrainManager.TrainPendingTransponder Data) {
            ...
            if (s >= 0) {
                if (Game.Sections[s].Exists(TrainManager.PlayerTrain)) {
                    // referencing the player's train's section
            ...
            //Game.AddMessage("SetBeaconData (type=" + data.Type.ToString() + ", signal=" + data.Signal.ToString() + ", distance=" + data.Distance.ToString("0.0000") + ", optional=" + data.Optional.ToString() + ")", Game.MessageDependency.None, Game.MessageType.Game, Game.MessageColor.Magenta, Game.SecondsSinceMidnight + 5.0);
