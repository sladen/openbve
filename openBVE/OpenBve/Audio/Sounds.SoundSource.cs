...
namespace OpenBve {
	internal static partial class Sounds {
		...
		internal class SoundSource {
			...
			/// <summary>The sound power in watts.</summary>
			...
			internal SoundSource(SoundBuffer buffer, double power, double pitch, double volume, OpenBveApi.Math.Vector3 position, TrainManager.Train train, int car, bool looped) {
				this.Buffer = buffer;
				this.Power = power;
				this.Pitch = pitch;
				this.Volume = volume;
