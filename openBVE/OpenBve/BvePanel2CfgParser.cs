...
namespace OpenBve {
    internal static class BvePanel2CfgParser {
        ...
        internal static void ParsePanel2Config(string TrainPath, System.Text.Encoding Encoding, TrainManager.Train Train) {
            ...
            World.ColorRGB PanelTransparentColor = new World.ColorRGB(0, 0, 255);
            ...
            { // camera restriction
                ...
                World.CameraRestrictionBottomLeft = new World.Vector3D(x0 * WorldWidth, y0 * WorldHeight, EyeDistance);
                World.CameraRestrictionTopRight = new World.Vector3D(x1 * WorldWidth, y1 * WorldHeight, EyeDistance);
                Train.Cars[0].DriverYaw = Math.Atan((PanelCenterX - PanelOriginX) * WorldWidth / PanelResolution);
                Train.Cars[0].DriverPitch = Math.Atan((PanelOriginY - PanelCenterY) * WorldWidth / PanelResolution);
            }
            // create panel
            if (PanelDaytimeImage != null) {
                if (!System.IO.File.Exists(PanelDaytimeImage)) {
                    ...
                    CreateElement(Train, 0.0, 0.0, PanelBitmapWidth, PanelBitmapHeight, 0.5, 0.5, 0.0, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday, tnight, new World.ColorRGBA(255, 255, 255, 255), false);
                }
            }
                        ...
                        switch (Section.ToLowerInvariant()) {
                            ...
                            case "pilotlamp": {
                                    ...
                                    if (DaytimeImage == null) {
                                        Interface.AddMessage(Interface.MessageType.Error, false, "DaytimeImage is a required to be specified in " + Section + " in " + FileName);
                                    }
                                    // create element
                                    if (DaytimeImage != null) {
                                        ...
                                        int j = CreateElement(Train, LocationX, LocationY, w, h, 0.5, 0.5, (double)Layer * StackDistance, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday, tnight, new World.ColorRGBA(255, 255, 255, 255), false);
                                        string f = GetStackLanguageFromSubject(Train, Subject, Section + " in " + FileName);
                                        Train.Cars[0].Sections[0].Elements[j].StateFunction = FunctionScripts.GetFunctionScriptFromPostfixNotation(f + " 1 == --");
                                    }
                                } break;
                            // needle
                            case "needle": {
                                    ...
                                    double LocationX = 0.0, LocationY = 0.0;
                                    string DaytimeImage = null, NighttimeImage = null;
                                    World.ColorRGBA Color = new World.ColorRGBA(255, 255, 255, 255);
                                    World.ColorRGB TransparentColor = new World.ColorRGB(0, 0, 255);
                                    ...
                                    if (DaytimeImage != null) {
                                        ...
                                        if (NaturalFrequency >= 0.0 & DampingRatio >= 0.0) {
                                            Train.Cars[0].Sections[0].Elements[j].RotateZDamping = new ObjectManager.Damping(NaturalFrequency, DampingRatio);
                                        }
                                        Train.Cars[0].Sections[0].Elements[j].RotateZFunction = FunctionScripts.GetFunctionScriptFromPostfixNotation(f);
                                    }
                                } break;
                            // digitalnumber
                            case "digitalnumber": {
                                    ...
                                    if (Interval <= 0) {
                                        Interface.AddMessage(Interface.MessageType.Error, false, "Interval is a required to be specified in " + Section + " in " + FileName);
                                    }
                                    // create element
                                    if (DaytimeImage != null & Interval > 0) {
                                        ...
                                        for (int k = 0; k < tday.Length; k++) {
                                            int l = CreateElement(Train, LocationX, LocationY, (double)wday, (double)Interval, 0.5, 0.5, (double)Layer * StackDistance, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday[k], tnight[k], new World.ColorRGBA(255, 255, 255, 255), k != 0);
                                            if (k == 0) j = l;
                                        }
                                        string f = GetStackLanguageFromSubject(Train, Subject, Section + " in " + FileName);
                                        Train.Cars[0].Sections[0].Elements[j].StateFunction = FunctionScripts.GetFunctionScriptFromPostfixNotation(f);
                                    }
                                } break;
                            // digitalgauge
                            case "digitalgauge": {
                                    string Subject = "true";
                                    double LocationX = 0.0, LocationY = 0.0;
                                    World.ColorRGBA Color = new World.ColorRGBA(0, 0, 0, 255);
                                    double Radius = 0.0;
                                    int Layer = 0;
                                    double InitialAngle = -2.0943951023932, LastAngle = 2.0943951023932;
                                    double Minimum = 0.0, Maximum = 1000.0;
                                    ...
                                    if (InitialAngle < -3.1416 | InitialAngle > 3.1416) {
                                        Interface.AddMessage(Interface.MessageType.Warning, false, "InitialAngle should be between -180 and 180 for the current implementation in " + Section + " in " + FileName);
                                    }
                                    if (LastAngle < -3.1416 | LastAngle > 3.1416) {
                                        Interface.AddMessage(Interface.MessageType.Warning, false, "LastAngle should be between -180 and 180 for the current implementation in " + Section + " in " + FileName);
                                    }
                                    if (Radius != 0.0) {
                                        // create element 
                                        ...
                                        InitialAngle = 0.0174532925199433 * InitialAngle + Math.PI;
                                        LastAngle = 0.0174532925199433 * LastAngle + Math.PI;
                                        ...
                                        Train.Cars[0].Sections[0].Elements[j].States[0].Object.Meshes[0] = new World.Mesh(new World.Vertex[] { new World.Vertex(cx, cy, cz), new World.Vertex(x3, y3, z3), new World.Vertex(x0, y0, z0) }, Color);
                                        Train.Cars[0].Sections[0].Elements[j].States[0].Object.Meshes[1] = new World.Mesh(new World.Vertex[] { new World.Vertex(cx, cy, cz), new World.Vertex(x0, y0, z0), new World.Vertex(x1, y1, z1) }, Color);
                                        Train.Cars[0].Sections[0].Elements[j].States[0].Object.Meshes[2] = new World.Mesh(new World.Vertex[] { new World.Vertex(cx, cy, cz), new World.Vertex(x1, y1, z1), new World.Vertex(x2, y2, z2) }, Color);
                                        Train.Cars[0].Sections[0].Elements[j].States[0].Object.Meshes[3] = new World.Mesh(new World.Vertex[] { new World.Vertex(cx, cy, cz), new World.Vertex(x2, y2, z2), new World.Vertex(x3, y3, z3) }, Color);
                                        Train.Cars[0].Sections[0].Elements[j].States[0].Object.Meshes[4] = new World.Mesh(new World.Vertex[] { new World.Vertex(cx, cy, cz), new World.Vertex(x3, y3, z3), new World.Vertex(x0, y0, z0) }, Color);
                                        ...
                                        Train.Cars[0].Sections[0].Elements[j].LEDFunction = FunctionScripts.GetFunctionScriptFromPostfixNotation(f);
                                    } else {
                                        Interface.AddMessage(Interface.MessageType.Error, false, "Radius is a required to be specified in " + Section + " in " + FileName);
                                    }
                                } break;
                            // timetable
                            case "timetable": {
        ...
        private static string GetStackLanguageFromSubject(TrainManager.Train Train, string Subject, string ErrorLocation) {
            ...
            { // detect d# suffix
                ...
                if (i >= 0 & i < Subject.Length - 1) {
                    if (string.Equals(Subject.Substring(i, 1), "d", StringComparison.InvariantCultureIgnoreCase)) {
                        int n; if (int.TryParse(Subject.Substring(i + 1), System.Globalization.NumberStyles.Integer, Culture, out n)) {
                            if (n == 0) {
                                Suffix = " floor 10 mod";
                            } else {
            ...
            // transform subject
            string Code; switch (Subject.ToLowerInvariant()) {
                case "true": Code = "1"; break;
                case "kmph": Code = "speedometer abs 3.6 *"; break;
                case "mph": Code = "speedometer abs 2.2369362920544 *"; break;
                case "ms": Code = "speedometer abs"; break;
                case "bc": Code = "brakecylinder 0.001 *"; break;
                case "mr": Code = "mainreservoir 0.001 *"; break;
                case "sap": Code = "straightairpipe 0.001 *"; break;
                case "bp": Code = "brakepipe 0.001 *"; break;
                case "er": Code = "equalizingreservoir 0.001 *"; break;
                case "door": Code = "1 doors -"; break;
                case "csc": Code = "handleconstspeed"; break;
                case "power":
                ...
                case "rev": Code = "handlereverser ++"; break;
                case "hour": Code = "0.000277777777777778 time * 24 mod floor"; break;
                case "min": Code = "0.0166666666666667 time * 60 mod floor"; break;
                case "sec": Code = "time 60 mod floor"; break;
                case "atc": Code = "atcstate"; break;
                default: {
                        Code = "0";
        ...
        private static int CreateElement(TrainManager.Train Train, double Left, double Top, double Width, double Height, double RelativeRotationCenterX, double RelativeRotationCenterY, double Distance, double PanelResolution, double PanelLeft, double PanelRight, double PanelTop, double PanelBottom, double PanelBitmapWidth, double PanelBitmapHeight, double PanelCenterX, double PanelCenterY, double PanelOriginX, double PanelOriginY, double DriverX, double DriverY, double DriverZ, int DaytimeTextureIndex, int NighttimeTextureIndex, World.ColorRGBA Color, bool AddStateToLastElement) {
            ...
            v[0] = new World.Vector3D(x0 - xm, y1 - ym, 0);
            v[1] = new World.Vector3D(x0 - xm, y0 - ym, 0);
            v[2] = new World.Vector3D(x1 - xm, y0 - ym, 0);
            v[3] = new World.Vector3D(x1 - xm, y1 - ym, 0);
            ...
            Object.Meshes[0].Materials[0].Flags = (byte)(DaytimeTextureIndex >= 0 ? World.MeshMaterial.TransparentColorMask : 0);
            Object.Meshes[0].Materials[0].Color = Color;
            Object.Meshes[0].Materials[0].TransparentColor = new World.ColorRGB(0, 0, 255);
            Object.Meshes[0].Materials[0].DaytimeTextureIndex = DaytimeTextureIndex;
            Object.Meshes[0].Materials[0].NighttimeTextureIndex = NighttimeTextureIndex;
            Object.Dynamic = 1;
            // calculate offset
            World.Vector3D o;
            o.X = xm + DriverX;
